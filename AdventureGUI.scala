  ...
  def top = new MainFrame {
    ...
    def playTurn(command: String) = {
      ...
      if (this.player.hasQuit) {
        ...
        if (turnReport.contains("foxbattle")) {
          this.foxBattle
        }
        else {
    ...
    def updateInfo(info: String) = {
      ...
      this.locationInfo.text = this.player.location.fullDescription + "\nYou have " + this.player.getLifePoints + " lifepoints."
      this.turnCounter.text = "Turns played: " + this.game.turnCount
    }

    
    // Possible ending that depends wether player has "energy drink" or not
    def foxBattle = {
      if (this.player.has("energy drink") && this.player.getLifePoints > 3) {
        this.updateInfo("Wait...What is happening?\nIs that.. A FOX!!!\nMaybe the energy drink could help me outrun the fox?\nYES!\nThanks to the help of the energy drink, you managed to escape the fox.")
      }
      else {
        if (this.player.getLifePoints < 2) {
          this.player.reducePoints
          this.updateInfo("Wait...What is happening?\nIs that.. A FOX!!!\nUnfortunately you ran out of energy and the Fox got you.\nYou lost the game.")
        }
        else {
          this.game.addFoods
          this.player.reducePoints
          this.updateInfo("Wait...What is happening?\nIs that.. A FOX!!!\nYou barely escaped the fox, but you couldn't fly south.\nYou also lost one lifepoint.\nYou should go eat more grass to gain more energy.\nPerhaps there's also something interesting in the forest.")
        }
      }
    }
